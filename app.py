...
def generate_plots(N, mu, sigma2, S):
    ...
    Y = errors  # Since we want no relationship, Y is just the error term
    ...
    plt.plot(X, model.predict(X_reshape), color='red', linewidth=2)
    plt.xlabel("X")
    plt.ylabel("Y")
    ...
    for _ in range(S):
        ...
        # Generate Y values with normal error
        errors_sim = np.random.normal(mu, sigma, N)
        Y_sim = errors_sim  # No relationship, just error term
